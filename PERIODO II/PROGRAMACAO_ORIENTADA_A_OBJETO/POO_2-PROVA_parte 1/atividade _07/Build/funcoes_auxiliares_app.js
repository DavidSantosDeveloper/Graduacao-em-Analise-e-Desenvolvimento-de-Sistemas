...
import { EntradaDeCaracteresDeTexto, EntradaNumerica, EntradaVazia, ContaInexistenteError } from "./Excecoes.js";
//>>>>>>>>>>>>>>>>>>MENU APP.JS<<<<<<<<<<<<<<<<<<<<<<
let input = prompt();
...
export function cadrastrar(banco) {
    try {
        ...
        let conta;
        if (tipo_da_conta == 0)
            conta = new Conta(numero, titular, Number(saldo));
        else
            conta = new Poupanca(numero, titular, Number(saldo), 0);
        banco.inserir(conta);
        console.log("\n");
        console.log(banco);
        console.log("\n");
...
// FUNCAO DA OPCAO 5
export function excluir_conta(banco) {
    try {
...
// FUNCAO DA OPCAO 6
export function realizar_deposito(banco) {
    try {
...
// FUNCAO DA OPCAO 8
export function render_juros_em_uma_conta_poupanca(banco) {
    try {
