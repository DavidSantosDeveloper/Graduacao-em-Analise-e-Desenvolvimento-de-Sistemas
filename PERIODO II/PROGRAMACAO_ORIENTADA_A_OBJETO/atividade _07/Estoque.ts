...
class Estoque {
    private lista_de_produtos:Produto[]=[]

    adicionarProduto(novo_produto:Produto){
        ...
        if( existencia_id_do_produto==true || existencia_nome_produto==true){
            ...
            if(novo_produto.verificarValidade()==false) return
        }

        this.lista_de_produtos.push(novo_produto)
    }
    consultarProduto(id_produto_procurado:string){
        let produto_procurado:Produto | null= null
    ...
    verificarExistenciaDeUmProduto(id_produto_procurado:string) : boolean{
        for (const produto_atual of this.lista_de_produtos) {
            if(produto_atual.getId()==id_produto_procurado){
                return true
            }
    ...
    verificarExistenciaDeUmNomeDeProduto(nome_de_produto:string){
        let resultado=false
    ...
    retornarIndiceDoProdutoNaLista(id_produto_procurado:string) : number{
    ...
    removerProduto(id_produto_a_ser_removido:string){
    ...
    reporProduto(id_produto_a_ser_reposto:string,quantidade:number){
    ...
    retirarProduto(id_produto_a_ser_reposto:string,quantidade:number){
        let posicao_do_produto_na_lista=this.retornarIndiceDoProdutoNaLista(id_produto_a_ser_reposto)
    ...
    listarProdutosVencidos(){
        let lista_de_produtos_vencidos:ProdutoPerecivel[]=[]

        for (const produto_atual of this.lista_de_produtos) {
            if(produto_atual instanceof ProdutoPerecivel ){
                if(produto_atual.verificarValidade()==false) lista_de_produtos_vencidos.push(produto_atual)
            }
        }
